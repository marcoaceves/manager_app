...
# total # of tasks
    ...
    def get_all_posts(cls):
        query = """SELECT ANY_VALUE(first_name) as first_name, ANY_VALUE(user2.id) as user_id, ANY_VALUE(posts.id) as id, ANY_VALUE(posts.created_at) as created_at, ANY_VALUE(posts.updated_at) as updated_at, count(post_id) as total_likes,  ANY_VALUE(content) as content
        FROM  users as user2 JOIN posts ON user2.id = posts.user_id LEFT JOIN likes ON posts.id = likes.post_id  GROUP BY created_at ORDER by ANY_VALUE(posts.id) DESC;"""
